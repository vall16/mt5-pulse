...
import requests
from models import LoginRequest, LoginResponse, ServerRequest, ServerReorderRequest, TraderServersUpdate,Trader, Newtrader,UserResponse, ServerResponse
...
def run_migrations():
    ...
    # Migration: aggiungi sort_order ai servers se manca
    for attempt in range(3):
        try:
            ...
            cursor.execute("SHOW COLUMNS FROM servers WHERE Field='sort_order'")
            if not cursor.fetchone():
                log("🔧 Migration: aggiungo colonna sort_order a servers...")
                cursor.execute("ALTER TABLE servers ADD COLUMN sort_order INT NULL")
                cursor.execute("UPDATE servers SET sort_order = id")
                conn.commit()
                log("✅ Migration completata: sort_order aggiunta")
            else:
                log("✅ Migration sort_order già applicata")
            cursor.close()
            conn.close()
            break
        except Exception as e:
            ...
            try:
                ...
                pass
            if attempt < 2:
                log(f"⚠️ Migration sort_order in attesa di lock, riprovo ({e})")
            else:
                log(f"⚠️ Migration sort_order saltata: {e}")

def ensure_servers_sort_order():
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("SET SESSION innodb_lock_wait_timeout = 10")
        ...
        cursor.execute("SHOW COLUMNS FROM servers WHERE Field='sort_order'")
        if not cursor.fetchone():
            log("🔧 Migration runtime: aggiungo colonna sort_order a servers...")
            cursor.execute("ALTER TABLE servers ADD COLUMN sort_order INT NULL")
            cursor.execute("UPDATE servers SET sort_order = id")
            conn.commit()
            log("✅ Migration runtime completata: sort_order aggiunta")
            return True
        cursor.close()
        conn.close()
    except Exception as e:
        log(f"⚠️ Migration runtime sort_order saltata: {e}")
    return False
...
def get_servers():
    try:
        ...
        cursor = conn.cursor(dictionary=True)
        ensure_servers_sort_order()
        cursor.execute("SELECT * FROM servers ORDER BY COALESCE(sort_order, id), id")
...
def insert_server(server: ServerRequest):
    ...
    try:
        ...
        query = """
            INSERT INTO servers 
            (`user`, `pwd`, `server`,`server_alias`, `platform`, `ip`, `path`, `port`, `is_active`, `sort_order`, `created_at`, `updated_at`)
...
@router.put("/servers/reorder")
def reorder_servers(reorder: ServerReorderRequest):
    conn = get_connection()
    if not conn:
        raise HTTPException(status_code=500, detail="Database connection failed")
    ...
    try:
        ...
        known = set(valid_ids)
        for sid in reorder.ordered_ids:
            if sid not in known:
                cursor.close()
                conn.close()
                raise HTTPException(status_code=404, detail=f"Server {sid} not found")

        for position, sid in enumerate(reorder.ordered_ids):
            cursor.execute(
                "UPDATE servers SET sort_order = %s WHERE id = %s",
                (position + 1, sid)
            )
        ...
        print("❌ Errore MySQL durante reorder server:", e)
        raise HTTPException(status_code=500, detail=f"MySQL Error: {e}")
    except Exception as e:
        print("❌ Errore generico durante reorder server:", e)
